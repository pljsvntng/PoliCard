    ...
    return (
      <div className="flex items-center gap-1.5 flex-1 min-w-0">
        <input
          ...
          className="flex-1 text-sm rounded-xl border border-teal px-2 py-1 min-w-0"
        />
        <button
          onClick={() => {
            if (draft.trim()) {
  ...
  return (
    <div>
      ...
      <div className="p-6 sm:p-8 max-w-4xl mx-auto">
        <div className="flex gap-1.5 mb-6 bg-ink/[0.04] p-1.5 rounded-2xl overflow-x-auto thin-scroll">
          {TABS.map(({ key, label, icon: Icon }) => (
            <button
              key={key}
              onClick={() => setTab(key)}
              className={`press-effect flex items-center gap-1.5 text-sm px-3.5 py-2 rounded-xl whitespace-nowrap font-medium transition-all duration-150 ${
                tab === key ? "bg-paper text-teal-deep shadow-[var(--shadow-soft)]" : "text-slate hover:text-ink"
              }`}
            >
              <Icon size={14} /> {label}
            </button>
